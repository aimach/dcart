...
const isSelectedMarker = (
  ...
  if (selectedMarker) {
    return (
      ...
      `${(selectedMarker as PointType).latitude}-${
        (selectedMarker as PointType).longitude
      }-${(selectedMarker as PointType).color}-${
        (selectedMarker as PointType).shape
      }`
    );
  }
  return false;
...
const getCreationAndModificationString = (
  ...
  let string = `${translation[language].common.createdOn} ${creationDate} ${
    translation[language].common.by
...
const getMapAttribution = (tileLayerURL: string): string => {
  switch (tileLayerURL) {
    case "https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png":
      return '<a href="https://www.openstreetmap.org/copyright" target="_blank" rel="noreferrer">OpenStreetMap</a> contributors';
    case "https://cawm.lib.uiowa.edu/tiles/%7Bz%7D/%7Bx%7D/%7By%7D.png/tiles/{z}/{x}/{y}.png":
      return '<a href="https://cawm.lib.uiowa.edu/index.html" target="_blank" rel="noreferrer">Consortium of Ancient World Mappers</a> contributors';
    case "https://{s}.basemaps.cartocdn.com/dark_nolabels/{z}/{x}/{y}{r}.png":
      return '<a href="https://carto.com/" target="_blank" rel="noreferrer">Carto</a> contributors';
    default:
      return "";
  }
};
