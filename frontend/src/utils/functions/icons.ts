...
// import des types
import type { Marker, MarkerCluster, MarkerOptions } from "leaflet";
...
const getLittleIcon = (
  ...
  if (!backgroundColor && (shape === "circle" || !shape)) {
    classNameString = `${style.circleBrownIcon} ${
      style[customClassName as string]
    }`;
  }
  const borderStyle =
...
const getNumberInsideIcon = (
  sourcesNb: number,
  isNbDisplayed: boolean,
  content?: string
) => {
  if (isNbDisplayed) {
    if (sourcesNb >= 2) return sourcesNb.toString();
  }
  if (content) {
    return content;
  }
...
const getCircleIcon = (
  ...
  const iconContent = getNumberInsideIcon(sourcesNb, isNbDisplayed, content);
  return `
...
const getShapedDivContent = (
  ...
  let customFillAndStroke = `fill=${customColor} stroke=${
    hasGrayScale ? "black" : tinycolor(customColor).darken(10).toString()
  }`;
  let customTextColor = tinycolor(customColor).isDark() ? "white" : "black";

  if (isSelected) {
    customFillAndStroke = `fill="white" stroke-width="10" stroke=${tinycolor(
      customColor
    )
      .darken(10)
      .toString()}`;
...
const getIcon = (
  ...
  let customIcon = getDefaultIcon(
    ...
    point.sources.length >= 2 ? point.sources.length.toString() : "",
    hasGrayScale
  );
  ...
  switch (shape) {
    case "circle": {
      const accessibleColor = hasGrayScale
        ...
        : `fill=${defaultColor}`;
      return `<svg xmlns="http://www.w3.org/2000/svg" ${size} ${xAndY} viewBox="0 0 100 100">${
        hasGrayScale && getPatternByColor(color as string)
      }<circle cx="50" cy="50" r="45" ${accessibleColor} stroke=${
        hasGrayScale ? "black" : "lightgrey"
      } stroke-width="5" /></svg>`;
    }
    case "square": {
      const accessibleColor = hasGrayScale
        ? `fill="url(#pattern-${color})"`
        : `fill=${defaultColor}`;
      return `<svg xmlns="http://www.w3.org/2000/svg" ${size} ${xAndY} viewBox="0 0 100 100"><rect x="5" y="5" width="90" height="90" ${accessibleColor} stroke=${
        hasGrayScale ? "black" : "lightgrey"
      } stroke-width="5"/>${
        hasGrayScale && getPatternByColor(color as string)
      }</svg>`;
    ...
    case "triangle": {
      const accessibleColor = hasGrayScale
        ? `fill="url(#pattern-${color})"`
        : `fill=${defaultColor}`;
      return `<svg xmlns="http://www.w3.org/2000/svg" ${size} ${xAndY} viewBox="0 0 100 100"><polygon points="50,10 90,90 10,90" ${accessibleColor} stroke=${
        hasGrayScale ? "black" : "lightgrey"
      } stroke-width="5" />${
        hasGrayScale && getPatternByColor(color as string)
      }</svg>`;
    ...
    case "diamond": {
      const accessibleColor = hasGrayScale
        ? `fill="url(#pattern-${color})"`
        : `fill=${defaultColor}`;
      return `<svg xmlns="http://www.w3.org/2000/svg" ${size} ${xAndY} viewBox="0 0 100 100"><polygon points="50,5 95,50 50,95 5,50" ${accessibleColor} stroke=${
        hasGrayScale ? "black" : "lightgrey"
      } stroke-width="5"/>${
        hasGrayScale && getPatternByColor(color as string)
      }</svg>`;
    ...
    case "star": {
      ...
      return `<svg xmlns="http://www.w3.org/2000/svg" ${size} ${xAndY} viewBox="0 0 100 100">${
        hasGrayScale && getPatternByColor(color as string)
      }<defs><filter id="blur" x="-5%" y="-5%" width="110%" height="110%"><feGaussianBlur in="SourceGraphic" stdDeviation="0.5"/></filter></defs><path d="
			M60,10 
			L71,42 
			L105,45 
			L78,65 
			...
			L49,42 
			Z"
			${accessibleColor} stroke=${
        hasGrayScale ? "black" : "lightgrey"
      } stroke-width="5"  filter="url(#blur)" stroke-linejoin="round"/></svg>`;
    }
    default: {
      const accessibleColor = hasGrayScale
        ? `fill="url(#pattern-${color})"`
...
const getBlendIconHTML = (
  ...
  if (markers.length === 2 || uniqueMarkersColorsAndShapes.length === 2) {
    ...
    let blendIcon = `<svg width="40" height="40" viewBox="0 0 40 40" xmlns="http://www.w3.org/2000/svg" stroke=${
      hasGrayScale ? "black" : "lightgrey"
    } stroke-width="1"><clipPath id="left-half">
...
function generateCamembertSVG(
  ...
  for (const color of arrayForLoop) {
    ...
    paths += `<path d="${d}" fill="${
      colors.length === 1
        ? (color as { color: string; shape: string }).color
        : color
    }" />`;
  ...
  return `
    <svg width="${size}" height="${size}" viewBox="0 0 ${size} ${size}" stroke=${
    hasGrayScale ? "black" : "lighgrey"
